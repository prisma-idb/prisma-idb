...
- **A model's own key is never _derived from_ its FK columns.** Even where a model's `@@id`/`@@unique` is itself compound (Phase 9.1/9.2 — IDB does support compound keys, contrary to an earlier draft of this ADR), that compound key is authored explicitly as its own attribute, not synthesized from a relation's FK fields. So a child model's own identity never depends on the excluded parent existing locally, whether the child's key is single-field or compound.
