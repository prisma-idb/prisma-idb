...
| 009 | [Foreign keys and referential actions](ADR%20009%20-%20FK%20Validation%20and%20Referential%20Action%20Enforcement.md)     | Accepted   | The client checks foreign keys on every write and runs `onDelete`/`onUpdate` actions itself. Both default to `restrict`, as in Postgres. |
