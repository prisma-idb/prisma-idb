...
**family-idb** is the main entry point for the `@prisma-idb` stack. It exposes the control-plane descriptor, the family instance, and the `prisma-idb` CLI for migration authoring and config integration. The former `prisma-idb` command remains an alias.
...
```bash
npm install @prisma-idb/family-idb
```
