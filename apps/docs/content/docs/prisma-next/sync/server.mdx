...
This page is the reference for the server side. For the setup sequence, start at [Sync](/docs/prisma-next/sync).
...
`Changelog` has to live on the server's real database — never IndexedDB, which is browser-only storage. `@prisma-next-idb/sync-server/postgres` derives it from your existing `schema.prisma` instead of requiring a hand-typed model:
...
`prepareSqlSchemaWithSync` (pure text transform, no file I/O — what this runs under the hood) and `injectChangelogModelSql` (just the `Changelog` append) are also exported individually from `@prisma-next-idb/sync-server/schema` if you need to compose this into a different schema-loading pipeline.
