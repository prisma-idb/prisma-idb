...
Everything above describes the **app space** — your own schema's migration history under `migrations/app/`. An IDB extension (like the [sync](/docs/prisma-next/sync) extension) contributes its own **extension space**: its own object stores, its own migration chain, versioned independently of your app space.
